EESchema Schematic File Version 4
LIBS:Bioreactor-cache
EELAYER 26 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title "BioReactor Controller"
Date "2019-02-14"
Rev "0.1"
Comp "WWU"
Comment1 "ESP32 connected to all sensors"
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L ESP32:ESP32-DEVKITC-V1 U1
U 1 1 5C66149E
P 5850 3750
F 0 "U1" H 5850 4917 50  0000 C CNN
F 1 "ESP32-DEVKITC-V1" H 5850 4826 50  0000 C CNN
F 2 "MODULE_ESP32-DEVKITC-32D" H 7100 4000 50  0001 L BNN
F 3 "Espressif Systems" H 7300 3900 50  0001 L BNN
F 4 "None" H 7350 3800 50  0001 L BNN "Field4"
F 5 "None" H 7600 3400 50  0001 L BNN "Field5"
F 6 "ESP32-DEVKITC-32D" H 7250 4200 50  0001 L BNN "Field6"
F 7 "Eval Board For Esp-Wroom-32" H 7350 3700 50  0001 L BNN "Field7"
F 8 "Unavailable" H 7400 3250 50  0001 L BNN "Field8"
	1    5850 3750
	1    0    0    -1  
$EndComp
Wire Notes Line
	7950 850  7950 5900
Wire Notes Line
	7950 5900 10600 5900
Wire Notes Line
	10600 5900 10600 850 
Wire Notes Line
	10600 850  7950 850 
Wire Notes Line
	1150 1150 3900 1150
Wire Notes Line
	3900 1150 3900 7200
Wire Notes Line
	3900 7200 1050 7200
Wire Notes Line
	1050 7200 1050 1200
$Comp
L power:+5V #PWR0101
U 1 1 5C661920
P 4800 4550
F 0 "#PWR0101" H 4800 4400 50  0001 C CNN
F 1 "+5V" H 4815 4723 50  0000 C CNN
F 2 "" H 4800 4550 50  0001 C CNN
F 3 "" H 4800 4550 50  0001 C CNN
	1    4800 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	4800 4550 5050 4550
$Comp
L Connector:Jack-DC J1
U 1 1 5C661A20
P 4650 5550
F 0 "J1" H 4705 5875 50  0000 C CNN
F 1 "Jack-DC" H 4705 5784 50  0000 C CNN
F 2 "" H 4700 5510 50  0001 C CNN
F 3 "~" H 4700 5510 50  0001 C CNN
	1    4650 5550
	1    0    0    -1  
$EndComp
$Comp
L power:+24V #PWR0102
U 1 1 5C661C87
P 5550 5450
F 0 "#PWR0102" H 5550 5300 50  0001 C CNN
F 1 "+24V" H 5565 5623 50  0000 C CNN
F 2 "" H 5550 5450 50  0001 C CNN
F 3 "" H 5550 5450 50  0001 C CNN
	1    5550 5450
	1    0    0    -1  
$EndComp
Wire Wire Line
	4950 5450 5550 5450
$Comp
L Regulator_Linear:LM7824_TO220 U2
U 1 1 5C66273A
P 6150 5350
F 0 "U2" H 6150 5592 50  0000 C CNN
F 1 "LM7824_TO220" H 6150 5501 50  0000 C CNN
F 2 "Package_TO_SOT_THT:TO-220-3_Vertical" H 6150 5575 50  0001 C CIN
F 3 "http://www.fairchildsemi.com/ds/LM/LM7805.pdf" H 6150 5300 50  0001 C CNN
	1    6150 5350
	1    0    0    -1  
$EndComp
Wire Wire Line
	5550 5450 5850 5450
Wire Wire Line
	5850 5450 5850 5350
Connection ~ 5550 5450
Wire Wire Line
	4950 5650 5550 5650
$Comp
L power:GND #PWR0103
U 1 1 5C66295B
P 5550 5650
F 0 "#PWR0103" H 5550 5400 50  0001 C CNN
F 1 "GND" H 5555 5477 50  0000 C CNN
F 2 "" H 5550 5650 50  0001 C CNN
F 3 "" H 5550 5650 50  0001 C CNN
	1    5550 5650
	1    0    0    -1  
$EndComp
Connection ~ 5550 5650
Wire Wire Line
	5550 5650 6150 5650
$Comp
L power:GND #PWR0104
U 1 1 5C662975
P 4400 4350
F 0 "#PWR0104" H 4400 4100 50  0001 C CNN
F 1 "GND" H 4405 4177 50  0000 C CNN
F 2 "" H 4400 4350 50  0001 C CNN
F 3 "" H 4400 4350 50  0001 C CNN
	1    4400 4350
	1    0    0    -1  
$EndComp
Wire Wire Line
	4400 4350 4400 4200
Wire Wire Line
	4400 4200 4950 4200
Wire Wire Line
	4950 4200 4950 4450
Wire Wire Line
	4950 4450 5050 4450
NoConn ~ 5050 4350
NoConn ~ 6650 4450
$Comp
L power:+3V3 #PWR0105
U 1 1 5C662DF5
P 7100 4550
F 0 "#PWR0105" H 7100 4400 50  0001 C CNN
F 1 "+3V3" H 7115 4723 50  0000 C CNN
F 2 "" H 7100 4550 50  0001 C CNN
F 3 "" H 7100 4550 50  0001 C CNN
	1    7100 4550
	1    0    0    -1  
$EndComp
Wire Wire Line
	6650 4550 7100 4550
Wire Wire Line
	6450 5350 6800 5350
$Comp
L power:+5V #PWR0106
U 1 1 5C662FCF
P 6800 5350
F 0 "#PWR0106" H 6800 5200 50  0001 C CNN
F 1 "+5V" H 6815 5523 50  0000 C CNN
F 2 "" H 6800 5350 50  0001 C CNN
F 3 "" H 6800 5350 50  0001 C CNN
	1    6800 5350
	1    0    0    -1  
$EndComp
$Comp
L Sensor_Temperature:MAX31855KASA U?
U 1 1 5C663262
P 2750 2200
F 0 "U?" H 2750 2778 50  0000 C CNN
F 1 "MAX31855KASA" H 2750 2687 50  0000 C CNN
F 2 "Package_SO:SOIC-8_3.9x4.9mm_P1.27mm" H 3750 1850 50  0001 C CIN
F 3 "http://datasheets.maximintegrated.com/en/ds/MAX31855.pdf" H 2750 2200 50  0001 C CNN
	1    2750 2200
	1    0    0    -1  
$EndComp
Wire Wire Line
	2750 1800 2750 1650
Wire Wire Line
	2750 1650 2600 1650
$Comp
L power:+3V3 #PWR?
U 1 1 5C66358E
P 2600 1650
F 0 "#PWR?" H 2600 1500 50  0001 C CNN
F 1 "+3V3" H 2615 1823 50  0000 C CNN
F 2 "" H 2600 1650 50  0001 C CNN
F 3 "" H 2600 1650 50  0001 C CNN
	1    2600 1650
	1    0    0    -1  
$EndComp
Wire Wire Line
	2750 2600 2750 2800
$Comp
L power:GND #PWR?
U 1 1 5C6636B5
P 2750 2800
F 0 "#PWR?" H 2750 2550 50  0001 C CNN
F 1 "GND" H 2755 2627 50  0000 C CNN
F 2 "" H 2750 2800 50  0001 C CNN
F 3 "" H 2750 2800 50  0001 C CNN
	1    2750 2800
	1    0    0    -1  
$EndComp
Text GLabel 3650 2250 2    50   Input ~ 0
_CS
Wire Wire Line
	3650 2250 3300 2250
Wire Wire Line
	3300 2250 3300 2300
Wire Wire Line
	3300 2300 3150 2300
Text GLabel 4650 4050 0    50   Input ~ 0
_CS
Wire Wire Line
	4650 4050 5050 4050
Text GLabel 4350 3950 0    50   Input ~ 0
MISO
Wire Wire Line
	4350 3950 5050 3950
Text GLabel 4650 3850 0    50   Input ~ 0
MOSI
Wire Wire Line
	4650 3850 5050 3850
Text GLabel 4350 3750 0    50   Input ~ 0
SCK
Wire Wire Line
	4350 3750 5050 3750
Text GLabel 3550 2100 2    50   Input ~ 0
MISO
Wire Wire Line
	3550 2100 3150 2100
Text GLabel 3450 1950 2    50   Input ~ 0
SCK
Wire Wire Line
	3450 1950 3250 1950
Wire Wire Line
	3250 1950 3250 2000
Wire Wire Line
	3250 2000 3150 2000
NoConn ~ 5050 4150
NoConn ~ 5050 4250
NoConn ~ 6650 4250
NoConn ~ 6650 4350
NoConn ~ 5050 2950
NoConn ~ 5050 2850
NoConn ~ 5050 3050
NoConn ~ 6650 3050
NoConn ~ 6650 3150
NoConn ~ 6650 3650
NoConn ~ 6650 3750
Text GLabel 4700 3150 0    50   Input ~ 0
SDA
Wire Wire Line
	4700 3150 4800 3150
Text GLabel 4450 3250 0    50   Input ~ 0
SCL
Wire Wire Line
	4450 3250 4500 3250
$Comp
L Connector_Generic:Conn_01x03 J?
U 1 1 5C665E9C
P 3200 3550
F 0 "J?" H 3120 3225 50  0000 C CNN
F 1 "Conn_01x03" H 3120 3316 50  0000 C CNN
F 2 "" H 3200 3550 50  0001 C CNN
F 3 "~" H 3200 3550 50  0001 C CNN
	1    3200 3550
	-1   0    0    1   
$EndComp
$Comp
L Connector_Generic:Conn_01x03 J?
U 1 1 5C665F97
P 2600 3550
F 0 "J?" H 2680 3592 50  0000 L CNN
F 1 "Conn_01x03" H 2680 3501 50  0000 L CNN
F 2 "" H 2600 3550 50  0001 C CNN
F 3 "~" H 2600 3550 50  0001 C CNN
	1    2600 3550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5C6660E7
P 3600 3750
F 0 "#PWR?" H 3600 3500 50  0001 C CNN
F 1 "GND" H 3605 3577 50  0000 C CNN
F 2 "" H 3600 3750 50  0001 C CNN
F 3 "" H 3600 3750 50  0001 C CNN
	1    3600 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	3400 3650 3600 3650
Wire Wire Line
	3600 3650 3600 3750
Wire Wire Line
	2400 3450 2300 3450
Wire Wire Line
	2300 3450 2300 3400
$Comp
L power:+5V #PWR?
U 1 1 5C666C91
P 2300 3400
F 0 "#PWR?" H 2300 3250 50  0001 C CNN
F 1 "+5V" H 2315 3573 50  0000 C CNN
F 2 "" H 2300 3400 50  0001 C CNN
F 3 "" H 2300 3400 50  0001 C CNN
	1    2300 3400
	1    0    0    -1  
$EndComp
NoConn ~ 2400 3550
NoConn ~ 2400 3650
Wire Wire Line
	3400 3550 3600 3550
Wire Wire Line
	3400 3450 3600 3450
Text GLabel 3600 3450 2    50   Input ~ 0
SCL
Text GLabel 3600 3550 2    50   Input ~ 0
SDA
$Comp
L Device:R R?
U 1 1 5C66988B
P 4800 3000
F 0 "R?" H 4870 3046 50  0000 L CNN
F 1 "4.7K" H 4870 2955 50  0000 L CNN
F 2 "" V 4730 3000 50  0001 C CNN
F 3 "~" H 4800 3000 50  0001 C CNN
	1    4800 3000
	1    0    0    -1  
$EndComp
Connection ~ 4800 3150
Wire Wire Line
	4800 3150 5050 3150
$Comp
L Device:R R?
U 1 1 5C669A43
P 4400 2900
F 0 "R?" H 4470 2946 50  0000 L CNN
F 1 "4.7K" H 4470 2855 50  0000 L CNN
F 2 "" V 4330 2900 50  0001 C CNN
F 3 "~" H 4400 2900 50  0001 C CNN
	1    4400 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	4400 3050 4400 3150
Wire Wire Line
	4400 3150 4450 3150
Wire Wire Line
	4450 3150 4450 3200
Wire Wire Line
	4450 3200 4500 3200
Wire Wire Line
	4500 3200 4500 3250
Connection ~ 4500 3250
Wire Wire Line
	4500 3250 5050 3250
Wire Wire Line
	4800 2850 4800 2750
Wire Wire Line
	4800 2750 4600 2750
$Comp
L power:+5V #PWR?
U 1 1 5C66ADE2
P 4600 2750
F 0 "#PWR?" H 4600 2600 50  0001 C CNN
F 1 "+5V" H 4615 2923 50  0000 C CNN
F 2 "" H 4600 2750 50  0001 C CNN
F 3 "" H 4600 2750 50  0001 C CNN
	1    4600 2750
	1    0    0    -1  
$EndComp
Connection ~ 4600 2750
Wire Wire Line
	4600 2750 4400 2750
Text GLabel 4950 3350 0    50   Input ~ 0
AMPLITUDE
Text GLabel 4900 3450 0    50   Input ~ 0
PHASE
Text GLabel 4900 3550 0    50   Input ~ 0
TRIGGER
Wire Wire Line
	4900 3550 5050 3550
Wire Wire Line
	4900 3450 5050 3450
Wire Wire Line
	4950 3350 5050 3350
$Comp
L Connector_Generic:Conn_01x04 J?
U 1 1 5C66D8D8
P 2850 4750
F 0 "J?" H 2770 5067 50  0000 C CNN
F 1 "Conn_01x04" H 2770 4976 50  0000 C CNN
F 2 "" H 2850 4750 50  0001 C CNN
F 3 "~" H 2850 4750 50  0001 C CNN
	1    2850 4750
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5C66FD92
P 3300 4600
F 0 "#PWR?" H 3300 4350 50  0001 C CNN
F 1 "GND" H 3300 4650 50  0000 C CNN
F 2 "" H 3300 4600 50  0001 C CNN
F 3 "" H 3300 4600 50  0001 C CNN
	1    3300 4600
	1    0    0    -1  
$EndComp
Wire Wire Line
	3050 4650 3200 4650
Wire Wire Line
	3200 4650 3200 4600
Wire Wire Line
	3200 4600 3300 4600
$Comp
L power:+5V #PWR?
U 1 1 5C6717D8
P 3550 4750
F 0 "#PWR?" H 3550 4600 50  0001 C CNN
F 1 "+5V" H 3565 4923 50  0000 C CNN
F 2 "" H 3550 4750 50  0001 C CNN
F 3 "" H 3550 4750 50  0001 C CNN
	1    3550 4750
	1    0    0    -1  
$EndComp
Wire Wire Line
	3050 4750 3550 4750
Text GLabel 3250 4850 2    50   Input ~ 0
SDA
Text GLabel 3250 4950 2    50   Input ~ 0
SCL
Wire Wire Line
	3050 4850 3250 4850
Wire Wire Line
	3050 4950 3250 4950
NoConn ~ 5050 3650
$Comp
L Connector:Conn_Coaxial J?
U 1 1 5C6767FE
P 3100 5450
F 0 "J?" H 3030 5688 50  0000 C CNN
F 1 "Conn_Coaxial" H 3030 5597 50  0000 C CNN
F 2 "" H 3100 5450 50  0001 C CNN
F 3 " ~" H 3100 5450 50  0001 C CNN
	1    3100 5450
	-1   0    0    -1  
$EndComp
$Comp
L Connector:Conn_Coaxial J?
U 1 1 5C67696E
P 3100 6100
F 0 "J?" H 3030 6338 50  0000 C CNN
F 1 "Conn_Coaxial" H 3030 6247 50  0000 C CNN
F 2 "" H 3100 6100 50  0001 C CNN
F 3 " ~" H 3100 6100 50  0001 C CNN
	1    3100 6100
	-1   0    0    -1  
$EndComp
$Comp
L Connector:Conn_Coaxial J?
U 1 1 5C6769B0
P 3100 6800
F 0 "J?" H 3030 7038 50  0000 C CNN
F 1 "Conn_Coaxial" H 3030 6947 50  0000 C CNN
F 2 "" H 3100 6800 50  0001 C CNN
F 3 " ~" H 3100 6800 50  0001 C CNN
	1    3100 6800
	-1   0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5C6769F4
P 2750 5700
F 0 "#PWR?" H 2750 5450 50  0001 C CNN
F 1 "GND" H 2750 5750 50  0000 C CNN
F 2 "" H 2750 5700 50  0001 C CNN
F 3 "" H 2750 5700 50  0001 C CNN
	1    2750 5700
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5C676A17
P 2800 6350
F 0 "#PWR?" H 2800 6100 50  0001 C CNN
F 1 "GND" H 2800 6400 50  0000 C CNN
F 2 "" H 2800 6350 50  0001 C CNN
F 3 "" H 2800 6350 50  0001 C CNN
	1    2800 6350
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 5C676A3A
P 2750 7050
F 0 "#PWR?" H 2750 6800 50  0001 C CNN
F 1 "GND" H 2750 7100 50  0000 C CNN
F 2 "" H 2750 7050 50  0001 C CNN
F 3 "" H 2750 7050 50  0001 C CNN
	1    2750 7050
	1    0    0    -1  
$EndComp
Wire Wire Line
	3100 5650 2750 5650
Wire Wire Line
	2750 5650 2750 5700
Wire Wire Line
	3100 6300 2800 6300
Wire Wire Line
	2800 6300 2800 6350
Wire Wire Line
	3100 7000 2750 7000
Wire Wire Line
	2750 7000 2750 7050
Text GLabel 3450 5450 2    50   Input ~ 0
AMPLITUDE
Text GLabel 3500 6100 2    50   Input ~ 0
PHASE
Text GLabel 3500 6800 2    50   Input ~ 0
TRIGGER
Wire Wire Line
	3300 6800 3500 6800
Wire Wire Line
	3300 6100 3500 6100
Wire Wire Line
	3300 5450 3450 5450
$EndSCHEMATC
